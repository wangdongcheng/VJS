...
STKCODE, STKNAME, STKCODE+' | '+STKNAME AS STK_CN,
 ...
 CASE WHEN 
	(CASE WHEN STK_USRNUM1=0 OR STK_USRNUM1 IS NULL THEN 0 ELSE ROUND(FLOOR((S3.STK_USRNUM5*Z.[3M_AVG])-LOC.STK_PHYSICAL-STK_ORDER_IN)/STK_USRNUM1,0)*STK_USRNUM1 END) < 0 THEN 0 ELSE 
		CASE WHEN STK_USRNUM1=0 OR STK_USRNUM1 IS NULL THEN 0 ELSE ROUND(FLOOR((S3.STK_USRNUM5*Z.[3M_AVG])-LOC.STK_PHYSICAL-STK_ORDER_IN)/STK_USRNUM1,0)*STK_USRNUM1 END END AS AUTO_ORDER_QTY,
 CASE WHEN 
	(CASE WHEN STK_USRNUM1=0 OR STK_USRNUM1 IS NULL THEN 0 ELSE ROUND(FLOOR((S3.STK_USRNUM5*Z.[3M_AVG])-LOC.STK_PHYSICAL-STK_ORDER_IN)/STK_USRNUM1,0)*STK_USRNUM1 END) < 0 THEN 0 ELSE 
 ...
 LEFT JOIN (
	...
	where det_date between cast(getdate()-91 as date) and cast(getdate()-1 as date) and det_type in ('INV','CRN') AND DET_LEDGER = 'SL' AND DET_STKSORTKEY3 = @stktype AND STK_USRFLAG3 = 0 AND STK_USRFLAG2 = 1
 ...
 LEFT JOIN (
	...
	from SL_PL_NL_DETAIL 
	INNER JOIN STK_STOCK3 S3 ON S3.STKCODE3=DET_STOCK_CODE
	where det_date between cast(getdate()-366 as date) and cast(getdate()-1 as date) and det_type in ('INV','CRN') AND DET_LEDGER = 'SL' AND DET_STKSORTKEY3 = @stktype AND STK_USRFLAG3 = 0 AND STK_USRFLAG2 = 1
