SELECT AC.CUCODE AS 'Customer Code', AC.CUNAME AS 'Name', AC.CUUSER1 AS 'Default salesrep Code', '' AS 'Sales Manager',
...
SELECT AC.CUCODE AS 'Customer Code', AC.CUNAME AS 'Name', AC.CUUSER1 AS 'Default salesrep Code',
    AC.CUCONTACT AS 'Main Contact Attention', AC.CU_EMAIL AS 'Email', AC.CUPHONE AS 'Phone1',
    AC.CUADDRESS AS 'Billing Address L1', AC.CU_ADDRESS_USER1 AS 'Billing City', AC.CUPOSTCODE AS 'Billing Postal Code',
    AD.AD_ADDRESS AS 'Shipping Address L1', AD.AD_ADDRESS_USER1 AS 'Shipping City',
    AD.AD_POSTCODE AS 'Shipping Postal Code', AC.CU_COUNTRY_CODE AS 'Country Code',
    CASE WHEN AC.CU_TERMS = 'CASH' OR AC.CU_TERMS = 'CASH SALE' THEN 'COD'
        WHEN UPPER(LTRIM(RTRIM(AC.CU_TERMS))) LIKE '% DAYS' 
        THEN CONCAT(
            CASE WHEN LEN(LEFT(AC.CU_TERMS, CHARINDEX(' ', AC.CU_TERMS + ' ') - 1)) = 1 THEN '0' ELSE '' 
...
SELECT AC.CUCODE AS 'Customer Code', AC.CUNAME AS 'Name', AC.CUUSER1 AS 'Default salesrep Code',
    AC.CUCONTACT AS 'Main Contact Attention', AC.CU_EMAIL AS 'Email', AC.CUPHONE AS 'Phone1',
    AC.CUADDRESS AS 'Billing Address L1', AC.CU_ADDRESS_USER1 AS 'Billing City', AC.CUPOSTCODE AS 'Billing Postal Code',
    CASE WHEN AD.AD_DEL_ADDRESS = 1 THEN 'Delivery Address' 
    WHEN AD.AD_DEL_ADDRESS_2 = 1 THEN 'Delivery Address'
    WHEN AD.AD_INV_ADDRESS = 1 THEN 'Invoice Address'
    WHEN AD.AD_STAT_ADDRESS = 1 THEN 'Statement Address' END AS AddressType,
    AD.AD_ADDRESS AS 'Shipping Address L1', AD.AD_ADDRESS_USER1 AS 'Shipping City',
    AD.AD_POSTCODE AS 'Shipping Postal Code', AC.CU_COUNTRY_CODE AS 'Country Code',
    CASE WHEN AC.CU_TERMS = 'CASH' OR AC.CU_TERMS = 'CASH SALE' THEN 'COD'
        WHEN UPPER(LTRIM(RTRIM(AC.CU_TERMS))) LIKE '% DAYS' 
...
FROM SL_ACCOUNTS AC
    INNER JOIN SL_ACCOUNTS2 AC2 ON AC.CUCODE=AC2.CUCODE2
    INNER JOIN SL_ADDRESSES AD ON AC.CUCODE=AD.AD_ACC_CODE
WHERE AC.CU_DO_NOT_USE = 0 AND AD_DO_NOT_USE = 0
ORDER BY 2;
